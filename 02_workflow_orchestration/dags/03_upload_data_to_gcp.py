...
destination_dir = "/home/airflow/files/GCP/"
...
def unzip_file(zip_dir, zip_name):
    with ZipFile(zip_dir + zip_name, 'r') as zip_file:
        zip_file.extractall(zip_dir)
        print(f"File {zip_dir + zip_name.replace("zip", "csv")} extracted succesfully from {zip_dir + zip_name}.")
...
def upload_file(file, bucket_name, local_dir):
    ...
    blob = bucket.blob(file.replace(local_dir, ""))
    blob.upload_from_filename(file)
    if storage.Blob(bucket=bucket, name=file.replace(local_dir, "")).exists(client):
...
unzip_data = PythonOperator(
    ...
    op_kwargs = {"zip_dir": destination_dir, "zip_name": file_name_zipped},
    dag = dag
)
